...
export const productValidationSchema = yup.object().shape({
  ...
  quantity: yup.number().when('is_external', {
  ...
  external_product_button_text: yup.string().when('is_external', {
    is: (value: boolean) => value,
    then: (schema) =>
      schema.required('form:error-external_product_button_text-required'),
    otherwise: (schema) =>
      schema
        .transform((value) => (isNaN(value) ? undefined : value))
        .notRequired(),
  }),
  external_product_url: yup.string().when('is_external', {
    is: (value: boolean) => value,
    then: (schema) =>
      schema.required('form:error-external-product-url-required'),
    otherwise: (schema) =>
      schema
        .transform((value) => (isNaN(value) ? undefined : value))
        .notRequired(),
